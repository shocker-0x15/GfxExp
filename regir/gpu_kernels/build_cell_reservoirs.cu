...
CUDA_DEVICE_FUNCTION CUDA_INLINE void buildCellReservoirsAndTemporalReuse(uint32_t frameIndex) {
    ...
    if constexpr (useTemporalReuse) {
        ...
        float weight = lengthCorrection * prevReservoir.getSumWeights(); // Assume the new target PDF and the prev target PDF are the same here (but this is actually TODO).
