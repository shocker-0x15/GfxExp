...
namespace shared {
    ...
    struct HitPointParams {
        ...
        uint32_t qGeometricNormalInWorld;
        uint32_t qTexCoord0DirInWorld;
        uint32_t qTexCoord;
        uint32_t matSlot;
        uint32_t instSlot;
    ...
    public:
        CUDA_COMMON_FUNCTION void initialize(const SampleType &initSample) {
            m_sample = initSample;
            m_sumWeights = 0;
            m_streamLength = 0;
        }
        ...
        CUDA_COMMON_FUNCTION SampleType getSample() const {
            return m_sample;
        }
    ...
    union SampleVisibility {
        uint32_t asUInt;
        struct {
            uint32_t newSample : 1;
            uint32_t newSampleOnTemporal : 1;
            uint32_t newSampleOnSpatiotemporal : 1;
            uint32_t temporalPassedHeuristic : 1;
            uint32_t temporalSample : 1;
            uint32_t temporalSampleOnCurrent : 1;
            uint32_t temporalSampleOnSpatiotemporal : 1;
            uint32_t spatiotemporalPassedHeuristic : 1;
            uint32_t spatiotemporalSample : 1;
            uint32_t spatiotemporalSampleOnCurrent : 1;
            uint32_t spatiotemporalSampleOnTemporal : 1;
            uint32_t selectedSample : 1;
        };
    ...
    struct GBuffer3Elements {
        uint32_t qShadingNormal;
        uint32_t qShadingTangent;
        uint32_t qTexCoord;
        uint32_t matSlot;
    };
    ...
    struct StaticPipelineLaunchParameters {
        ...
        optixu::NativeBlockBuffer2D<GBuffer3Elements> GBuffer3[2];
        ...
        optixu::BlockBuffer2D<Reservoir<LightSample>, 0> reservoirBufferArray[2];
        optixu::NativeBlockBuffer2D<ReservoirInfo> reservoirInfoBufferArray[2];
        optixu::NativeBlockBuffer2D<SampleVisibility> sampleVisibilityBufferArray[2];
    ...
    struct PerFramePipelineLaunchParameters {
        ...
        uint32_t log2NumCandidateSamples : 4;
        uint32_t numSpatialNeighbors : 4;
        uint32_t useLowDiscrepancyNeighbors : 1;
        uint32_t reuseVisibility : 1;
        uint32_t reuseVisibilityForTemporal : 1; // only for rearchitected ver.
        uint32_t reuseVisibilityForSpatiotemporal : 1; // only for rearchitected ver.
        uint32_t enableTemporalReuse : 1;
        uint32_t enableSpatialReuse : 1;
        uint32_t useUnbiasedEstimator : 1;
    ...
    struct PipelineLaunchParameters {
        ...
        uint32_t currentReservoirIndex : 1;
        uint32_t spatialNeighborBaseIndex : 10;
    };
...
CUDA_DEVICE_FUNCTION CUDA_INLINE bool testNeighbor(
    ...
    const GBuffer0Elements nbGb0Elems = plp.s->GBuffer0[nbBufIdx].read(nbCoord);
    if (nbGb0Elems.instSlot == 0xFFFFFFFF)
        return false;

    if constexpr (testGeometry) {
        const GBuffer2Elements nbGb2Elems = plp.s->GBuffer2[nbBufIdx].read(nbCoord);
        const GBuffer3Elements nbGb3Elems = plp.s->GBuffer3[nbBufIdx].read(nbCoord);
        const Point3D nbPositionInWorld = nbGb2Elems.positionInWorld;
        const Normal3D nbNormalInWorld = decodeNormal(nbGb3Elems.qShadingNormal);
        const float nbDist = length(plp.f->camera.position - nbPositionInWorld);
